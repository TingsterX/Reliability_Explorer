...
for (p1 in 1:length(pipelines)){
    fname <- paste0(out_dir, '/dbICC_allconn_', pnames[p1], '_2subsets_', amount ,'.csv')
    df1 <- fread(fname)
    for (p2 in 1:length(pipelines)){
    	fname <- paste0(out_dir, '/dbICC_allconn_', pnames[p2], '_2subsets_', amount ,'.csv')
    	df2 <- fread(fname)
...
fout <- paste0(out_dir, '/dbICC_allconn_pipeines_comp_', amount ,'.csv')
fwrite(df.out, fout)
